...
async function instrumentWithSentry(cb) {
    if (process.env.SENTRY_ENV && process.env.SENTRY_ENV !== "development") {
        ...
        logWarn(
            "Skipping uploading/creating Sentry source maps. (development build)",
        );
    }
...
function zipDist() {
    ...
    return src(`${DIST_DIR}/**`, {
        ...
        .on("end", () =>
            log(`Zip file created: ${path.join(PACKAGE_DIR, zipFileName)}`),
        );
}
...
// Main build task
const zipProdBuild = series(
    removeExtraDatabaseDir,
    instrumentWithSentry,
    zipDist,
);
