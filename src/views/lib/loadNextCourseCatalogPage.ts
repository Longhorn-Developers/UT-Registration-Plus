 ...
 */
export type AutoLoadStatusType =
    (typeof AutoLoadStatus)[keyof typeof AutoLoadStatus];
 ...
 */
export async function loadNextCourseCatalogPage(): Promise<
    [AutoLoadStatusType, HTMLTableRowElement[]]
> {
...
export function removePaginationButtons(doc: Document) {
    const nextButton = doc.querySelectorAll<HTMLAnchorElement>(
        NEXT_PAGE_BUTTON_SELECTOR,
    );
    nextButton.forEach((button) => button.remove());
    const prevButton = doc.querySelectorAll<HTMLAnchorElement>(
        PREV_PAGE_BUTTON_SELECTOR,
    );
