...
function processQueryResult(res: QueryExecResult): Required<CourseSQLRow> {
    ...
    for (let i = 0; i < res.columns.length; i++) {
        ...
        switch (col) {
            ...
            case "F":
            case "Other":
                row[col] = res.values.reduce(
                    (acc, cur) => acc + (cur[i] as number),
                    0,
                ) as never;
                break;
            default:
 ...
 */
export async function queryAggregateDistribution(
    course: Course,
): Promise<[Distribution, Semester[], boolean]> {
    ...
    if (!instructorIncluded || !res?.columns?.length) {
        ...
        const [queryWithoutInstructor, paramsWithoutInstructor] = generateQuery(
            course,
            null,
            false,
        );
    ...
    const rawSemesters = res.values.reduce(
        (acc, cur) => acc.add(cur[0] as string),
        new Set<string>(),
    );
 ...
 */
export async function querySemesterDistribution(
    course: Course,
    semester: Semester,
): Promise<[Distribution, boolean]> {
    const db = await initializeDB();
...
export class NoDataError extends Error {
    constructor(course: Course) {
        super(
            `No data for #${course.uniqueId} ${course.department} ${course.number}`,
        );
