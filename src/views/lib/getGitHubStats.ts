 ...
 */
export type LD_SWE_GITHUB_USERNAMES =
    (typeof LONGHORN_DEVELOPERS_SWE)[number]["githubUsername"];

/**
 * Represents the GitHub usernames of the admins in the LONGHORN_DEVELOPERS_ADMINS array.
 */
export type LD_ADMIN_GITHUB_USERNAMES =
    (typeof LONGHORN_DEVELOPERS_ADMINS)[number]["githubUsername"];
...
export type UTRP_LEAD_GITHUB_USERNAMES =
    (typeof UTRP_LEADS)[number]["githubUsername"];
...
export type UTRP_ALUMNI_GITHUB_USERNAMES =
    (typeof UTRP_ALUMNI)[number]["githubUsername"];
...
export class GitHubStatsService {
    ...
    private async fetchContributorStats(): Promise<
    ...
    > {
        ...
        const data = await this.fetchWithRetry(() =>
            this.fetchGitHub(CONTRIBUTORS_API_ROUTE),
        );
    ...
    private async fetchContributorNames(
        usernames: string[],
        ...
        for (const [login, stats] of Object.entries(allStats)) {
            if (
                LONGHORN_DEVELOPERS_ADMINS.some(
                    (admin) => admin.githubUsername === login,
                )
            ) {
