...
export default function useSchedules(): [
    ...
    const [activeSchedule, setActiveSchedule] = useState<UserSchedule>(
        schedules[activeIndex] ?? errorSchedule,
    );
    ...
    useEffect(() => {
        ...
        const l2 = UserScheduleStore.subscribe(
            "activeIndex",
            ({ newValue }) => {
 ...
 */
export async function updateCourseColors(
    courseID: number,
    primaryColor: HexColor,
) {
    const activeSchedule = getActiveSchedule();
    const updatedCourseIndex = activeSchedule.courses.findIndex(
        (c) => c.uniqueId === courseID,
    );
    ...
    try {
        const { colorway: primaryColorWay, index: primaryIndex } =
            getColorwayFromColor(primaryColor);
        const { secondaryColor: colorFromWay } = getCourseColors(
            primaryColorWay,
            primaryIndex,
            400,
        );
