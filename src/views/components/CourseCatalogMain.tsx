...
interface Props {
    support: Extract<
        SiteSupportType,
        "COURSE_CATALOG_DETAILS" | "COURSE_CATALOG_LIST"
    >;
}
 ...
 */
export default function CourseCatalogMain({
    support,
}: Props): JSX.Element | null {
    const [rows, setRows] = React.useState<ScrapedRow[]>([]);
    const [selectedCourse, setSelectedCourse] = useState<Course | null>(null);
    const [showPopup, setShowPopup] = useState(false);
    const [enableScrollToLoad, setEnableScrollToLoad] =
    ...
    useEffect(() => {
        ...
        prevCourseTitleRef.current =
            scrapedRows
                ...
                ?.element.querySelector(".course_header")?.textContent ?? null;
    }, [support]);
    ...
    const addRows = (newRows: ScrapedRow[]) => {
        newRows.forEach((row) => {
            const courseTitle =
                row.element.querySelector(".course_header")?.textContent ??
                null;
            if (row.course === null) {
