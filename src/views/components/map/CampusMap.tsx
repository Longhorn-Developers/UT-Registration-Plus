...
export default function CampusMap({
    ...
    const [hoveredPathIndex, setHoveredPathIndex] = useState<number | null>(
        null,
    );
    const [toggledPathIndex, setToggledPathIndex] = useState<number | null>(
        null,
    );
    ...
    const [showPrioritizedOnly, setShowPrioritizedOnly] =
    ...
    const getDailyPaths = useCallback((courses: ProcessInPersonMeetings[]) => {
        const sortedCourses = [...courses].sort(
            (a, b) => a.normalizedStartTime - b.normalizedStartTime,
        );
    ...
    const relevantPaths = useMemo(() => {
        ...
        const coursesForDay = processedCourses.filter(
            (course) => course.day === DAY_MAPPING[selectedDay],
        );
    ...
    const getNodeSize = useCallback(
        (type: NodeType): number => {
            ...
            const zoomRatio =
                (zoomLevel - ZOOM_LEVELS.LOW) /
                (ZOOM_LEVELS.HIGH - ZOOM_LEVELS.LOW);
    ...
    return (
        ...
        >
            ...
            <div className="absolute right-8 top-8 z-10 max-h-[calc(100vh-120px)] flex flex-col gap-4 overflow-y-auto">
                ...
                {selected.start && selected.end && (
                    <PathStats startId={selected.start} endId={selected.end} />
                )}
