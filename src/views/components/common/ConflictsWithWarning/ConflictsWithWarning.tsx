...
export default function ConflictsWithWarning({
    ...
    return (
        <div className='min-w-21 w-21 flex flex-col items-start gap-2.5 rounded bg-[#AF2E2D] p-2.5'>
            <ConflictsWithoutWarningText>Conflicts With:</ConflictsWithoutWarningText>
            <ConflictsWithoutWarningText>{UniqueCourseConflictText}</ConflictsWithoutWarningText>
        </div>
    );
}
...
function ConflictsWithoutWarningText({ children }: { children: string }) {
    return (
        <Text variant='mini' as='span' className='text-white'>
            {children}
        </Text>
    );
}
