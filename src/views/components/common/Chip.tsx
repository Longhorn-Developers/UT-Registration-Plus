...
export function Chip({
    ...
    const longName =
        Object.entries(labelMap).find(
            ([_full, short]) => short === label,
        )?.[0] ?? label;
    ...
    return (
        <Text
            ...
            variant="h4"
            className={clsx(
                "min-w-5 inline-flex items-center justify-center gap-2.5 rounded-lg px-1.5 py-0.5",
                {
            ...
            title={
                variant === "flag"
                    ? `${longName} flag`
                    : `${longName} core curriculum requirement`
            }
        >
            {label}
        </Text>
