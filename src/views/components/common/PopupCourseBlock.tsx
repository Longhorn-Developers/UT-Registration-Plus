...
const CourseMeeting = memo(
    ({
        ...
        meeting: Course["schedule"]["meetings"][0];
        fontColor: string;
    }) => {
        ...
        return (
            <Text
                key={dateString}
                className={clsx("flex-1 truncate select-none", fontColor)}
                variant="h3-course"
            >
        ...
        >
            {IS_STORYBOOK ? (
                ...
                >
                    <DotsSixVertical
                        weight="bold"
                        className="h-6 w-6 cursor-move text-white"
                    />
                </div>
            ) : (
