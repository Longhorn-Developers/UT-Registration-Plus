...
export default function TableRow({
    ...
    const [container, setContainer] = useState<HTMLTableCellElement | null>(
        null,
    );
    ...
    const [highlightConflicts, setHighlightConflicts] =
    ...
    useEffect(() => {
        ...
        element.classList[isInSchedule ? "add" : "remove"](
            styles.inActiveSchedule!,
        );
    ...
    useEffect(() => {
        ...
        for (const c of activeSchedule.courses) {
            if (
                c.uniqueId !== course.uniqueId &&
                course.getConflicts(c).length > 0
            ) {
                conflicts.push(c);
            }
        }
        ...
        if (highlightConflicts) {
            element.classList[conflicts.length ? "add" : "remove"](
                styles.isConflict!,
            );
        } else {
            element.classList[conflicts.length ? "add" : "remove"](
                styles.isConflictNoLineThrough!,
            );
        }
