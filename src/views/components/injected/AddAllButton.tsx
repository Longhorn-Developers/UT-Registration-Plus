...
export default function InjectedButton(): JSX.Element | null {
    ...
    const extractCoursesFromCalendar = async () => {
        ...
        const calendarElement = Array.from(tableElements).find((el) =>
            el.textContent?.includes("12PM"),
        );
        ...
        const anchorTags = Array.from(
            calendarElement.querySelectorAll("a"),
        ).filter((anchor) => !anchor.href.includes("google.com"));

        // Make sure to remove duplicate anchorTags using set
        const uniqueAnchorTags = Array.from(
            new Set(anchorTags.map((a) => a.href)),
        );
    ...
    useEffect(() => {
        ...
        const targetElement = Array.from(targetElements).find((el) =>
            el.textContent?.includes("My Information"),
        );

        if (targetElement) {
            console.debug(
                "[UTRP] Injecting AddAllButton into My Information region",
            );
            ...
            console.debug(
                "[UTRP] My Information region not found, AddAllButton not injected",
            );
        }
    }, []);
