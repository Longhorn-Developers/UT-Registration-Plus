...
export default function PopupMain(): JSX.Element {
    ...
    useEffect(() => {
        setFunny((prevFunny) => {
            ...
            const splashTextWithoutCurrent = splashText.filter(
                (text) => text !== prevFunny,
            );
            const randomIndex = Math.floor(
                Math.random() * splashTextWithoutCurrent.length,
            );
        ...
        <div className="h-screen max-h-full flex flex-col bg-white">
            ...
            {activeSchedule?.courses?.length === 0 && (
                <div className="max-w-64 flex flex-col items-center self-center gap-1.25 px-2 py-2 pt-24">
                    <Text
                        variant="p"
                        className="text-center text-ut-gray !font-normal"
                    >
                        {funny}
                    </Text>
            ...
            >
                {activeSchedule?.courses?.length > 0 && (
                    <SortableList
                        ...
                        onChange={(reordered) => {
                            activeSchedule.courses = reordered.map(
                                ({ course }) => course,
                            );
                            replaceSchedule(
                                getActiveSchedule(),
